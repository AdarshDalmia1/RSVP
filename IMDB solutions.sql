...
SELECT 
	SUM(id IS NULL) AS nulls_in_id, 
	SUM(title IS NULL) AS nulls_in_title, 
	SUM(year IS NULL) AS nulls_in_year,
	SUM(date_published IS NULL) AS nulls_in_date_published,
	SUM(duration IS NULL) AS nulls_in_duration,
	SUM(country IS NULL) AS nulls_in_country,
	SUM(worlwide_gross_income IS NULL) AS nulls_in_worldwide_gross_income,
	SUM(languages IS NULL) AS nulls_in_languages,
	SUM(production_company IS NULL) AS nulls_in_production_company
FROM movie;
...
-- Query to count the number of movies released each year.
select year, count(id) as number_of_movies
from movie
group by year
order by year;

-- Query to count the number of movies released each month.
...
SELECT MAX(total_votes) AS max_total_votes, languages
FROM movie AS m
INNER JOIN ratings AS r ON m.id = r.movie_id
